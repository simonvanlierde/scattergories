...
class ArgosProvider:
    ...
    def __post_init__(self) -> None:
        ...
        # Loaded lazily so warm cache-hit runs never touch the network.
        self.available_packages: list[Any] | None = None
        self.installed_pairs: set[tuple[str, str]] = set()

    def _refresh_available_packages(self) -> list[Any]:
        """Refresh the Argos package index on first use."""
    ...
    def ensure_pair(self, from_locale: str, to_locale: str) -> None:
        ...
        try:
            ...
            translation = None
        # Argos returns None (no exception) when the pair package is absent.
        if translation is not None:
            self.installed_pairs.add((from_locale, to_locale))
            return

        available_packages = self._refresh_available_packages()
