...
Every dependency is locked (`../pnpm-lock.yaml`, [`tools/uv.lock`](../tools/uv.lock)) and every tool version is pinned via [`mise.toml`](../mise.toml), so a clean checkout builds identically. The same gates run locally, in pre-push hooks ([`lefthook.yml`](../lefthook.yml)), and in [CI](../.github/workflows/ci.yml):
